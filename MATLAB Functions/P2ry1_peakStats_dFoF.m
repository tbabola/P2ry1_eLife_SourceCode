    ...
    for i=1:leftEventNum
        ...
        if rightEvent
            ...
            if size(uniqVals,1) > 2
                ...
                rightLabel(rightLabel == preserveVal) = 600;
            end
...
function [eventStat] = eventStats(rightEvents, leftEvents, biEvents, smRIC, smLIC, leftBinary, rightBinary)
 ...
 %           %half-width measurements
            ...
            pkindex = find(pks == leftAmp);
            eventStat(index).hwt = w(pkindex);
...
%
        ...
        [pks,locs,w] = findpeaks(smIC(eventStat(end).xloc,windowStart:windowEnd),'WidthReference','halfheight');
        pkindex = find(pks == eventStat(end).domAmp);
        eventStat(index).hwt = w(pkindex);
